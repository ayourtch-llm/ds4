...
#define DS4_METAL_PROFILE_Q_STAGE(name) do { \
    ...
    /* Try f16 heads path: fused inv_rope+f16 conversion + strided output
     * projection GEMM. Requires q8 f16 weight cache for attn_output_a.
     * Fall back to f32 inv_rope + packed output projection if unavailable. */
    {
        int heads_f16_ok = 0;
        if (ok && !g->quality && n_tokens >= 2048 && g->batch_q_f16 &&
            getenv("DS4_CUDA_NO_HEADS_F16") == NULL) {
            int rope_ok = ds4_gpu_inv_rope_f16_tensor(g->batch_q_f16,
                                               g->batch_heads,
                                               n_tokens,
                                               DS4_N_HEAD,
                                               DS4_N_HEAD_DIM,
                                               ...
                                               DS4_ROPE_YARN_BETA_SLOW);
            if (rope_ok) {
                heads_f16_ok = ds4_gpu_attention_output_q8_batch_tensor(g->batch_attn_out,
                                                                    g->batch_attn_low,
                                                                    g->batch_group_tmp,
                                                                    g->batch_low_tmp,
                                                                    model->map,
