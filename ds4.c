...
#define DS4_METAL_PROFILE_Q_STAGE(name) do { \
    ...
    /* Same gating as the f16-Q path (see ds4.c above): quality mode disables
     * the f16 Q8 weight cache used by the strided output-projection GEMM, and
     * cuBLAS f16-out kernel selection regresses below 2K. Use the f16-heads
     * fast path only when both conditions are met. */
    if (ok && !g->quality && n_tokens >= 2048 && g->batch_q_f16 &&
